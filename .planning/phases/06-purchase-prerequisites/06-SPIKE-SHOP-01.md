...
| "Filter or group data in Analytics" | ~~**OFF**~~ → **ON** | **SUPERSEDED 2026-08-13.** This row recorded the value as OFF; it was measured **ON** on both definitions during `06-02` Task 3, so the OFF here was intent, not observed state. Andrew's decision on being shown it: leave it ON. The original reasoning is retained below because it is still the tradeoff being accepted, not a refuted claim. ~~Both fields are approved non-PHI per `CLAUDE.md` rule 6, but pushing a health-adjacent completion flag into Shopify Analytics segmentation is what converts an approved non-PHI flag into a problem — it makes "people who completed an allergy assessment" a targetable audience inside a system with no BAA. **Keep it off.**~~ **Do not treat "Analytics filter OFF" as a precondition in `06-03` or `06-06`.** Full record in `06-02-SUMMARY.md` Task 3 Deviation. |
